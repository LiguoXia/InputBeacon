...
& (Join-Path $frameworkDirectory 'csc.exe') /nologo /target:winexe /platform:anycpu /warnaserror+ /codepage:65001 /main:InputBeacon.LivePreview "/out:$runner" /reference:System.dll /reference:System.Core.dll /reference:System.Drawing.dll /reference:System.Windows.Forms.dll /reference:Accessibility.dll "/reference:$(Join-Path $frameworkDirectory 'WPF\UIAutomationClient.dll')" "/reference:$(Join-Path $frameworkDirectory 'WPF\UIAutomationTypes.dll')" "/reference:$(Join-Path $frameworkDirectory 'WPF\WindowsBase.dll')" "/win32manifest:$(Join-Path $projectRoot 'app.manifest')" @sources (Join-Path $PSScriptRoot 'LivePreview.cs')

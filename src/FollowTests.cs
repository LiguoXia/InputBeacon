...
namespace InputBeacon
{
    internal static class FollowTests
    {
        ...
        private static void CheckFocusHints(Action<string, bool> check)
        {
            ...
            tick(0);
            check("Native input focus displays on the first valid sample", lifetime.TriggerCount == 1 && lifetime.ShouldShow(true, 2, true, 0));
            tick(100);
            tick(200);
            check("Entering a text field gets one complete countdown", lifetime.TriggerCount == 1 && lifetime.ShouldShow(true, 2, true, 1999) && !lifetime.ShouldShow(true, 2, true, 2000));
            ...
            tick(2300);
            check("Browser to WeChat focus displays immediately without a mode change", lifetime.TriggerCount == 2 && lifetime.ShouldShow(true, 2, true, 2300));
            tick(2500);
            check("Later caret polls do not extend the immediate focus hint", lifetime.ShouldShow(true, 2, true, 4299) && !lifetime.ShouldShow(true, 2, true, 4300));
            caret.InputIdentity = "wechat-field-b";
            tick(4600);
            check("Virtual fields still reject a single changed identity sample", lifetime.TriggerCount == 2);
            tick(4700);
            check("Virtual input focus confirms at the next poll without 200ms wait", lifetime.TriggerCount == 3 && lifetime.ShouldShow(true, 2, true, 4700));
            ...
            tick(12000);
            check("Slow caret discovery shows immediately when geometry arrives", lifetime.TriggerCount == 5 && lifetime.ShouldShow(true, 2, true, 12000));
            tick(12200);
            check("Slow caret discovery receives the full configured duration", lifetime.ShouldShow(true, 2, true, 13999) && !lifetime.ShouldShow(true, 2, true, 14000));
            ...
            caret = new CaretSample { Foreground = state.Foreground, Focus = state.Focus, Valid = true };
            tick(14000);
            check("Native controls trigger immediately without UIA support", lifetime.TriggerCount == 6 && lifetime.ShouldShow(true, 2, true, 14000));
